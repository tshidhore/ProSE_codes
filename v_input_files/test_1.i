...
# E = 0.0005 GPa, density = 1400 kg/m3, Gc = 300 J/m2
# Crack - 200 um at 30 deg in center of domain
# L0 = 40 um
...
[Functions]
  [./initial_crack]
    type = ParsedFunction
    value = 'min(exp(-abs(y-tan(30.0000*pi/180)*x-211.3249)/m)*if(max(m+1-abs(y-tan(30.0000*pi/180)*x-211.3249),0),1,0)*if(max(200.0000+10.0000-abs(y+cot(30.0000*pi/180)*x-1366.0254),0),1,0),1)'
    vals = 40
    vars = m
  [../]
  [./initial_gradient]
    type = ParsedFunction
    value = '(1/(m*m))*min(exp(-abs(y-tan(30.0000*pi/180)*x-211.3249)/m)*if(max(m+1-abs(y-tan(30.0000*pi/180)*x-211.3249),0),1,0)*if(max(200.0000+10.0000-abs(y+cot(30.0000*pi/180)*x-1366.0254),0),1,0),1)'
    vals = 40
    vars = m
  [../]
  [./rightbc_loading]
...
[BCs]
  [./left_loading]
    ...
    boundary = left
    variable = ux
    function = leftbc_loading
  [../]
  [./right_loading]
    type = FunctionPresetBC
    boundary = right
    variable = ux
    function = rightbc_loading
  [../]
[]
################################################################################
